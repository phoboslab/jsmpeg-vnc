...
// double elapsed = timer_meassure(elapsed) { <statements to meassure> }
#define timer_meassure(TIME) 0; \
	for( \
		__int64 TIME##__timer_start = 0; \
		TIME##__timer_start == 0 && (TIME##__timer_start = __timer_meassure_start()) != 0; \
		__timer_meassure_end(&TIME##__timer_start, &TIME) \
	)
